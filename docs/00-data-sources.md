...
| 국내 주가 | **KRX Open API** (1순위) / 네이버금융 (개발용 폴백) | 확정 |
| 미국 주가 | **Tiingo 무료 티어** | 확정 (개인용 전제) |

### 결정 기록 (2026-08-05)

| 결정 | 선택 | 영향 |
|---|---|---|
| 배포 범위 | **개인용 / 비공개** | Tiingo 무료 티어 사용 가능. 단 `PriceProvider` 어댑터로 분리해 공개 전환 시 환경변수만 교체 |
| 국내 주가 | **KRX 우선 + 네이버 폴백** | `PRICE_PROVIDER_KR` 환경변수로 전환. KRX 키 승인 전까지 네이버로 개발 |
| DART 키 | **미발급** | Phase 1을 키 불필요 구간(`packages/shared`)부터 착수 |

⚠️ 개인용 전제이므로 **이 서비스를 공개 배포하려면 주가 소스를 먼저 교체해야 한다.**
Tiingo·네이버 둘 다 공개 배포 근거가 없다. 공개 시점에 Tiingo Power(법인 $50/월) +
KRX Open API 조합으로 전환하면 라이선스가 정리된다.
...
### 3.4 라이선스 — 반드시 결정해야 할 것

미국 주가 무료 티어는 **셋 다 상업적/공개 이용을 명시적으로 금지**한다.
"내부 사용" "비상업" "표시(display) 금지"가 약관에 박혀 있다.
즉 **이 서비스를 인터넷에 공개하는 순간 세 곳 다 위반**이다.

선택지:

| 안 | 내용 | 비용 | 적합한 경우 |
|---|---|---|---|
| **A. 개인용** | Tiingo 무료 티어, 로컬/비공개 배포 | ₩0 | 본인이 쓸 도구 |
| **B. 공개 서비스** | Tiingo Power (개인 $30/월, 법인 $50/월) | ~$30~50/월 | 공개 웹서비스 |
| **C. 밸류에이션 제외** | PER/PBR 빼고 재무지표만 (DART+SEC는 둘 다 공개 배포 OK) | ₩0 | Phase 4를 나중으로 미룸 |

**권장: 지금은 A로 개발하되, 어댑터를 갈아끼우면 B로 전환되게 설계한다.**
`PriceProvider` 인터페이스를 두고 `TiingoProvider` / `KrxProvider` / `NaverProvider`를
구현체로 분리하면, 공개 시점에 환경변수만 바꿔서 전환할 수 있다.
그리고 UI 하단에 데이터 출처·라이선스 고지 영역을 처음부터 만들어 둔다.

### 3.5 국내 주가 권장안

**1순위: KRX Open API** — `https://data-dbg.krx.co.kr/svc/apis/sto/stk_bydd_trd`
(헤더 `AUTH_KEY`, 파라미터 `basDd`=YYYYMMDD). 공식·무료, 승인 약 1일 소요.
공식 소스라 라이선스 리스크가 없다는 게 결정적이다.
