...
| `FMP_API_KEY` | [financialmodelingprep.com](https://site.financialmodelingprep.com/) | 무료 250콜/일 | 애널리스트 추정 밴드 불가 |
...
**당시의 추정이 실제로 맞았는지**가 한 화면에서 읽힙니다.
...
쓰면 1월 결산인 엔비디아의 추정치가 한 해 뒤로 밀립니다. 액면분할 기준도 맞춥니다 — FMP
추정치는 현재 기준이라, 「액면분할 조정」을 끄면 추정치를 되돌려 올려야 실제선과 겹칩니다.

목표주가는 **현재 컨센서스만** 표시합니다. 발행일이 붙은 개별 목표주가는 유료 구간이고
(`price-target-news` 402), 구버전 `price-target` 엔드포인트는 2025-08-31자로 폐지됐습니다.
그래서 "그때 맞았나"는 목표주가가 아니라 추정치로 답합니다.

추정치도 목표주가도 예측이 아니라 애널리스트의 **의견**입니다. 당시의 정보와 판단이 섞여
있습니다. 화면에도 그렇게 적어 둡니다.

Tiingo와 같은 제약이 걸립니다 — 오히려 더 셉니다. FMP 약관은 데이터를
"제3자가 접근 가능한 도구나 애플리케이션에 통합"하는 것 자체를 금지합니다. 그래서
`FMP_API_KEY`는 배포판에 올리지 않고, 저장된 추정치도 배포용 덤프에서 빠집니다.
